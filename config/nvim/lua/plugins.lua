...
require("lazy").setup({
  ...
  "tpope/vim-liquid", {
    "othree/yajs.vim",
    ft = {
      "javascript", "javascript.jsx", "html", "typescript",
      "typescriptreact"
    }
  },
  ...
  { "moll/vim-node", ft = { "javascript", "typescript" } }, {
    ...
    config = function() vim.g.vim_jsx_pretty_highlight_close_tag = 1 end
  }, { "leafgarland/typescript-vim", ft = { "typescript", "typescript.tsx" } },
  { "wavded/vim-stylus", ft = { "stylus", "markdown" } },
  { "jxnblk/vim-mdx-js", ft = "mdx" }, { "groenewege/vim-less", ft = "less" },
  ...
  { "udalov/kotlin-vim", ft = "kotlin" }, {
    "elzr/vim-json",
    ft = "json",
    config = function() vim.g.vim_json_syntax_conceal = 0 end
  }, { "ekalinin/Dockerfile.vim", ft = "Dockerfile" }, "jparise/vim-graphql", {
  ...
  -- enable copilot support for Neovim
  {
    ...
    config = function()
      ...
      local copilot_node_command = vim.env.FNM_DIR ..
          "/aliases/copilot/bin/node"
      if utils.file_exists(copilot_node_command) then
        ...
        vim.cmd(
          'let g:copilot_node_command = "' .. copilot_node_command ..
          '"')
      end
    end

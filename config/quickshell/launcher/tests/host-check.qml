...
Scope {
    ...
    function hostChecks() {
        ...
        check(host.registryEntry("windows") === null, "window list retired in favor of the thumbnail switcher")
        wrapper.route("windows")
        check(host.scope === "", "retired windows route falls back to the launcher root")
        check(!host.rows.some(row => row.providerKey === "windows"), "root has no old window-list results")

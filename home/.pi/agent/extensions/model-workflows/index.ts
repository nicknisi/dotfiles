...
const WorkflowEnum = StringEnum(
  ["single", "parallel", "chain", "compare", "debate", "implement_with_review"] as const,
  {
...
const ParamsSchema = Type.Object({
  ...
  cwd: Type.Optional(
    Type.String({ description: "Working directory override for single workflow" }),
  ),
  ...
  synthesizer: Type.Optional(
    Type.String({ description: 'Synthesis agent. Default: "synthesizer".' }),
  ),
...
function formatToolCall(
  ...
  switch (toolName) {
    ...
    case "write":
      return (
        themeFg("muted", "write ") + themeFg("accent", shortenPath((args.path as string) || "..."))
      );
    case "edit":
      return (
        themeFg("muted", "edit ") + themeFg("accent", shortenPath((args.path as string) || "..."))
      );
    case "ls":
...
function renderWorkflowDetails(details: WorkflowDetails, expanded: boolean, theme: any): Component {
  ...
  if (results.length > 1) {
    container.addChild(
      new Text(
        theme.fg(
          "dim",
          `Progress: ${completed}/${results.length} complete${failed ? `, ${failed} failed` : ""}`,
        ),
        0,
        0,
      ),
  ...
  for (const result of results) {
    ...
    container.addChild(
      new Text(
        `${theme.fg("muted", "─── ")}${theme.fg("accent", title)} ${resultIcon}${theme.fg("dim", ` (${result.agent})`)}`,
        0,
        0,
      ),
    );
    ...
    const items = getDisplayItems(result.messages);
    const toolCalls = items.filter((item) => item.type === "toolCall") as Array<{
      type: "toolCall";
      name: string;
      args: Record<string, unknown>;
    }>;
    if (toolCalls.length > 0) {
      ...
      for (const toolCall of toolCalls) {
        container.addChild(
          new Text(
            theme.fg("muted", "→ ") +
              formatToolCall(toolCall.name, toolCall.args, theme.fg.bind(theme)),
            0,
            0,
          ),
        );
          ...
          if (message.role === "assistant") {
            ...
            if (message.usage) {
              ...
              currentResult.usage.contextTokens =
                message.usage.totalTokens || currentResult.usage.contextTokens;
            }
...
function buildCompareSynthesisTask(
  requestedTask: string,
  opusOutput: string,
  gptOutput: string,
): string {
...
function buildDebateRevisionTask(
  requestedTask: string,
  proposal: string,
  critique: string,
): string {
...
function buildDebateSynthesisTask(
  requestedTask: string,
  proposal: string,
  critique: string,
  revision: string,
...
async function executeWorkflow(
  ...
  const workflow: WorkflowName =
    params.workflow ??
    (params.chain?.length ? "chain" : params.tasks?.length ? "parallel" : "single");
  const results: SingleResult[] = [];
  const emit = () =>
    onUpdate?.(
      makeDetails(workflow, params.task, agentScope, discovery.projectAgentsDir, [...results]),
    );
  ...
  if (workflow === "single") {
    ...
    const result = await runSingleAgent(
      ...
      (partial) => {
        onUpdate?.(
          makeDetails(workflow, params.task, agentScope, discovery.projectAgentsDir, [partial]),
        );
      },
    );
    results.push(result);
  ...
  if (workflow === "parallel") {
    ...
    const completed = await mapWithConcurrencyLimit(
      params.tasks,
      MAX_CONCURRENCY,
      async (task, index) => {
        const result = await runSingleAgent(
          ...
          (partial) => {
            ...
            onUpdate?.(
              makeDetails(workflow, params.task, agentScope, discovery.projectAgentsDir, [
                ...allResults,
              ]),
            );
          },
  ...
  if (workflow === "compare") {
        ...
        (partial) => {
          ...
          onUpdate?.(
            makeDetails(
              ...
              agentScope,
              discovery.projectAgentsDir,
              parallelResults.filter(Boolean) as SingleResult[],
            ),
          );
        },
        entry.label,
    ...
    const synthesis = await runSingleAgent(
      ...
      (partial) =>
        onUpdate?.(
          makeDetails(workflow, task, agentScope, discovery.projectAgentsDir, [
            ...results,
            { ...partial, label: "Synthesis" },
          ]),
        ),
      "Synthesis",
    );
  ...
  if (workflow === "debate") {
    ...
    const proposal = await runSingleAgent(
      ctx.cwd,
      agents,
      proposer,
      buildDebateProposalTask(task),
      undefined,
      signal,
      (partial) => {
        onUpdate?.(
          makeDetails(workflow, task, agentScope, discovery.projectAgentsDir, [
            { ...partial, label: "Proposal" },
          ]),
        );
      },
      "Proposal",
    ...
    const revision = await runSingleAgent(
      ...
      agents,
      proposer,
      buildDebateRevisionTask(
        task,
        getFinalOutput(proposal.messages),
        getFinalOutput(critique.messages),
      ),
      undefined,
      signal,
      (partial) =>
    ...
    const synthesis = await runSingleAgent(
      ...
      agents,
      synthesizer,
      buildDebateSynthesisTask(
        task,
        getFinalOutput(proposal.messages),
        getFinalOutput(critique.messages),
        getFinalOutput(revision.messages),
      ),
      undefined,
      signal,
      (partial) =>
  ...
  const implementation = await runSingleAgent(
    ctx.cwd,
    agents,
    implementer,
    buildImplementationTask(task),
    undefined,
    signal,
    (partial) => {
      onUpdate?.(
  ...
  emit();
  if (isResultError(implementation))
    return makeDetails(workflow, task, agentScope, discovery.projectAgentsDir, results);
  ...
  const applied = await runSingleAgent(
    ...
    agents,
    implementer,
    buildApplyReviewTask(
      task,
      getFinalOutput(implementation.messages),
      getFinalOutput(review.messages),
    ),
    undefined,
    signal,
    (partial) =>
...
async function runWorkflowCommand(
  ...
  const execute = async (signal?: AbortSignal) =>
    executeWorkflow(ctx, agents, discovery, params, signal);
  ...
  if (ctx.hasUI) {
    details = await ctx.ui.custom<WorkflowDetails | null>((tui, theme, _kb, done) => {
      ...
      execute(loader.signal)
        .then(done)
        .catch((error) => {
          const failed = makeDetails(
            params.workflow ?? "single",
            params.task,
            agentScope,
            discovery.projectAgentsDir,
            [
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptSnippet:
      "Delegate work to named model agents with isolated context and compare or debate their answers.",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params: ToolParams, signal, onUpdate, ctx) {
      ...
      const details = await executeWorkflow(
        ctx,
        agents,
        discovery,
        params,
    ...
    renderCall(args: ToolParams, theme) {
      const workflow =
        args.workflow ??
        (args.chain?.length ? "chain" : args.tasks?.length ? "parallel" : "single");
      if (workflow === "single") {
        ...
        const preview = args.task
          ...
          : "...";
        return new Text(
          `${theme.fg("toolTitle", theme.bold(`${TOOL_NAME} `))}${theme.fg("accent", agent)}\n  ${theme.fg("dim", preview)}`,
          0,
          0,
        );
      }
  ...
  pi.registerCommand("ask-opus", {
    ...
    handler: async (args, ctx) => {
      ...
      await runWorkflowCommand(pi, ctx, "Asking Opus...", {
        workflow: "single",
        agent: "opus",
        task: args.trim(),
      });
  ...
  pi.registerCommand("ask-gpt", {
    ...
    handler: async (args, ctx) => {
      ...
      await runWorkflowCommand(pi, ctx, "Asking GPT...", {
        workflow: "single",
        agent: "gpt",
        task: args.trim(),
      });
  ...
  pi.registerCommand("compare-models", {
    ...
    handler: async (args, ctx) => {
      ...
      await runWorkflowCommand(pi, ctx, "Comparing models...", {
        workflow: "compare",
        task: args.trim(),
      });
    },
  ...
  pi.registerCommand("debate-models", {
    ...
    handler: async (args, ctx) => {
      ...
      await runWorkflowCommand(pi, ctx, "Debating models...", {
        workflow: "debate",
        task: args.trim(),
      });
    },

  ...
  for (const run of map.values()) {
    ...
    for (const child of run.children) {
      ...
      if (!existing) byKey.set(key, child);
        ...
        Object.assign(existing, Object.fromEntries(Object.entries(child).filter(([, value]) => value !== undefined)));
    }
...
function formatRunSummary(run: RunRef): string {
  ...
  for (const child of run.children) {
    ...
    if (child.sessionFile)
      lines.push(`  - history: history://${run.runId}${child.index !== undefined ? `/${child.index}` : ''}`);

...
function sessionInfo(file: string): {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (entry.type === "message" && entry.message?.role === "user" && !task)
        task = textFromBlocks(entry.message.content)
          .replace(/^Task:\s*/i, "")
          .trim();
...
function discoverAsyncRuns(map: Map<string, RunRef>) {
  const asyncRoot = path.join(
    os.tmpdir(),
    `pi-subagents-uid-${typeof process.getuid === "function" ? process.getuid() : "user"}`,
    "async-subagent-runs",
  );
  if (!fs.existsSync(asyncRoot)) return;
  let dirs: string[] = [];
  try {
    dirs = fs.readdirSync(asyncRoot).map((name) => path.join(asyncRoot, name));
  } catch {
    return;
  }
  for (const dir of dirs) {
    ...
    if (Array.isArray(status?.steps)) {
      status.steps.forEach((step: any, index: number) => {
        children.push({
          ...
          status: step.status,
          updatedAt:
            step.endedAt || step.lastActivityAt || status.lastUpdate || statMtime(statusFile),
        });
      });
    } else {
      children.push({
        ...
        status: status?.state,
        updatedAt:
          status?.endedAt || status?.lastActivityAt || status?.lastUpdate || statMtime(statusFile),
      });
    }
    ...
    upsertRun(map, {
      ...
      asyncDir: dir,
      statusFile,
      eventsFile: exists(path.join(dir, "events.jsonl"))
...
function discoverArtifactRuns(map: Map<string, RunRef>) {
  const files = walkFiles(path.join(AGENT_DIR, "sessions"), {
    maxDepth: 5,
    match: (file) =>
      /subagent-artifacts\/.+_(input|output|meta)\.md$|subagent-artifacts\/.+\.jsonl$|subagent-artifacts\/.+_meta\.json$/.test(
  ...
  for (const child of byBase.values()) {
    upsertRun(map, {
      ...
      children: [child as ChildRef],
    });
  }
}

function discoverChainRuns(map: Map<string, RunRef>) {
  const chainRoot = path.join(
    os.tmpdir(),
    `pi-subagents-uid-${typeof process.getuid === "function" ? process.getuid() : "user"}`,
    "chain-runs",
  );
  ...
  let dirs: string[] = [];
  try {
    dirs = fs.readdirSync(chainRoot).map((name) => path.join(chainRoot, name));
  } catch {
    return;
  }
  for (const dir of dirs) {
    ...
    const mdFiles = walkFiles(dir, {
      maxDepth: 4,
      maxFiles: 200,
      match: (file) => file.endsWith(".md") || file.endsWith(".json"),
    });
    upsertRun(map, {
      runId,
      source: "chain",
      ...
      children: mdFiles.map((file, index) => ({
        index,
        outputFile: file,
...
function filterRuns(runs: RunRef[], query?: string): RunRef[] {
  ...
  return runs.filter((run) =>
    [
      ...
      run.state,
      run.cwd,
      ...run.children.flatMap((c) => [c.agent, c.task, c.sessionFile, c.outputFile]),
    ]
      .filter(Boolean)
...
function formatRunList(runs: RunRef[], query?: string, limit = DEFAULT_LIMIT): string {
  ...
  const lines = [
    `Subagent runs (${filtered.length}${filterRuns(runs, query).length > filtered.length ? ` of ${filterRuns(runs, query).length}` : ""}):`,
  ];
  for (const run of filtered) {
    const labels = [run.source, run.mode, run.state].filter(Boolean).join("/");
    lines.push(
      `- agent://${run.runId} (${labels || "run"}, ${run.children.length} child${run.children.length === 1 ? "" : "ren"}, ${formatDate(run.updatedAt)})`,
    );
...
function resolveRun(uriHostOrId: string, runs: RunRef[]): RunRef {
  const matches = runs.filter(
    (run) => run.runId === uriHostOrId || run.runId.startsWith(uriHostOrId),
  );
  if (matches.length === 0)
    throw new Error(`No subagent run matched '${uriHostOrId}'. Try /agent list.`);
  if (matches.length > 1)
    throw new Error(
      `Ambiguous run id '${uriHostOrId}' matched: ${matches.map((r) => r.runId).join(", ")}`,
    );
  return matches[0];
}
...
function selectChild(run: RunRef, selector?: string): ChildRef[] {
  ...
  return run.children.filter(
    (child) => child.agent === selector || child.agent?.includes(selector),
  );
}
...
function formatRunSummary(run: RunRef): string {
  ...
  for (const child of run.children) {
    lines.push(
      `- ${child.index !== undefined ? `run-${child.index}` : "child"}${child.agent ? ` (${child.agent})` : ""}${child.status ? ` — ${child.status}` : ""}`,
    );
    if (child.task) lines.push(`  - task: ${short(child.task, 140)}`);
    if (child.sessionFile)
      lines.push(
        `  - history: history://${run.runId}${child.index !== undefined ? `/${child.index}` : ""}`,
      );
...
function parseAgentUri(uri: string): {
  ...
  if (!match)
    throw new Error(`Unsupported URI '${uri}'. Use agent://<runId> or history://<runId>.`);
...
function readAgentUri(uri: string, maxLines = DEFAULT_MAX_LINES): string {
  ...
  if (parsed.scheme === "history") {
    const children = selectChild(run, parsed.selector).filter(
      (child) => exists(child.sessionFile) || exists(child.jsonlFile),
    );
  ...
  return selected
    .map((child) => {
      ...
      if (leaf === "history" && child.sessionFile)
        return renderSession(child.sessionFile, maxLines);
      ...
      if (leaf === "jsonl" && (child.jsonlFile || child.sessionFile))
        return readTextFile(child.jsonlFile || child.sessionFile!, maxLines);
      if (leaf === "log" && child.logFile) return readTextFile(child.logFile, maxLines);
      if ((leaf === "output" || leaf === "result") && child.outputFile)
        return readTextFile(child.outputFile, maxLines);
      if ((leaf === "output" || leaf === "result") && child.logFile)
        return readTextFile(child.logFile, maxLines);
...
export default function agentUrls(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: "string",
          description: "Optional substring filter over run id, agent, task, cwd, and paths.",
        },
        limit: {
          type: "number",
    ...
    async execute(_id: string, params: { query?: string; limit?: number }) {
      ...
      const text = formatRunList(
        runs,
        params.query,
        Math.max(1, Math.min(params.limit ?? DEFAULT_LIMIT, 100)),
      );
      return {
        content: [{ type: "text", text }],
        details: {
          runs: filterRuns(runs, params.query).slice(0, params.limit ?? DEFAULT_LIMIT),
        },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description:
      "Read agent:// and history:// URLs for pi-subagents runs, outputs, and transcripts.",
    promptSnippet:
      "Read subagent outputs or transcripts via agent://<runId> and history://<runId>.",
    parameters: {
      type: "object",
      required: ["uri"],
      properties: {
        uri: {
          type: "string",
          description:
            "agent://<runId>[/child[/output|history|session|log|input|meta|jsonl]] or history://<runId>[/child]",
        },
        maxLines: {
          type: "number",
  ...
  pi.registerCommand("agent", {
    description:
      "Inspect pi-subagents URLs: /agent list [query] | /agent read agent://<runId> | /agent read history://<runId>",
    ...
    handler: async (args, ctx) => {
      ...
      if (["list", "ls"].includes(sub)) {
        postCommandResult(
          pi,
          ctx,
          formatRunList(discoverRuns(), rest.join(" ") || undefined, DEFAULT_LIMIT),
        );
        return;
      }
      if (["read", "show", "cat"].includes(sub)) {
        ...
        if (!uri) {
          ctx.ui.notify(
            "Usage: /agent read agent://<runId> or /agent read history://<runId>",
            "warning",
          );
          return;
        }
      ...
      ctx.ui.notify(
        "Usage: /agent list [query] | /agent read agent://<runId> | /agent read history://<runId>",
        "warning",
      );
    },
  });
